...
export class SessionService {
  ...
  showNewTokenAlert(): void {
    ...
    const toast = this.toastr.info(
      `Twoja sesja wygaśnie za <br><strong>${remainingTime} sekund</strong>.<br><u>Kliknij tutaj, aby ją przedłużyć.</u>`,
      'Sesja wygaśnie',
      {
        tapToDismiss: false,
        timeOut: 120000,
    ...
    const intervalId = setInterval(() => {
      ...
      if (toastElement) {
        ...
        if (messageElement) {
          messageElement.innerHTML = `Twoja sesja wygaśnie za <br><strong>${remainingTime} sekund</strong>.<br><u>Kliknij tutaj, aby ją przedłużyć.</u>`;
        }
      }
